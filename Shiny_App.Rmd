...
clean_data$state_full <- state_lookup[as.character(clean_data$state)]
...
# Generate bar graph
  ...
  output$plotly <- renderPlotly({
    ...
    clean_data <- clean_data[complete.cases(clean_data$state_full), ]
    
    #Using maps package, discovered through ChatGPT to add longitude and latitude for each state
      
    state.center <- as.data.frame(state.center)
    state.center$state_full <- state.name
    
    clean_data_merged <- merge(state.center, clean_data, by = "state_full")
    

    output$map <- renderLeaflet({
      
      filtered_data <- clean_data_merged
      if (input$map_gender != "All") {
      filtered_data <- filtered_data[filtered_data$Gender == input$map_gender,]
      }
      selected_variable <- input$map_variable
    filtered_data <- filtered_data[!is.na(filtered_data[[selected_variable]]),]
