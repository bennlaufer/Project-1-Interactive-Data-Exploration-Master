...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      selectInput("plot", "Select plot type:", choices = unique(c("map", "bar chart")))
    ),
    mainPanel(
      plotOutput("plot"),
  ...
  navbarPage(
    ...
    tabPanel("ggplot",
             sidebarPanel(
              ...
              selectInput("gender", "Select gender:", choices = c("All", unique(clean_data$Gender))),
              selectInput("variable", "Select variable:", choices = unique(names(clean_data)[2:ncol(clean_data)])),
              selectInput("plot", "Select plot type:", choices = unique(c("blank", "bar graph")))
          ),
          tableOutput("table"),
          plotOutput("ggplot")
    ),
    tabPanel("Scatter Plot",
             sidebarPanel(
              ...
              selectInput("gender", "Select gender:", choices = c("All", unique(clean_data$Gender))),
              selectInput("variable", "Select variable:", choices = unique(names(clean_data)[2:ncol(clean_data)])),
              selectInput("plot", "Select plot type:", choices = unique(c("map", "blank")))
          ),
          plotOutput("scatter"))
...
# Generate bar graph
  output$ggplot <- renderPlot({
    ...
    mapplot <- reactive({
    if(input$plot == "map" && input$country == "United States"){
...
server <- function(input, output) {
  ...
  # Define reactive values for color palette and bins
  pal <- reactiveValues()
  bins <- reactiveValues()
  ...
  observe({
    if(input$plot == "map" && input$country == "United States"){
      ...
      pal$pal <- colorBin("YlOrRd", domain = input$variable, bins = bins$bins)
      
      output$scatter <- renderLeaflet({
        m %>% addPolygons(
          data = your_polygon_data_here,
          lng = your_longitude_column_here,
          lat = your_latitude_column_here,
          fillColor = ~pal$pal(density),
          weight = 2,
          opacity = 1,
          color = "white",
          dashArray = "3",
