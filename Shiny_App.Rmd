  ...
  navbarPage(
    ...
    tabPanel("Plotly Graph",
             sidebarPanel(
              #selectInput("country", "Select country:", choices = c("All", unique(clean_data$Country))),
              selectInput("gender", "Select gender", choices = c("All", unique(clean_data$Gender))),
              selectInput("variable1", "Select first variable:", choices = unique(names(clean_data)[4:ncol(clean_data)])),
              selectInput("variable2", "Select second variable:", choices = unique(names(clean_data)[4:ncol(clean_data)])),
              #selectInput("plot", "Select plot type:", choices = unique(c("dot plot", "bar graph"))),
              ),
...
# Generate bar graph
  ...
  output$plotly <- renderPlotly({
      ...
      fig <- clean_data_summary() %>%
        plot_ly(
          type = 'scatter',
          mode = 'markers',
          x = as.formula(paste0("~", input$variable1),
          y = ~Percent,
          #text = ~Country,
          #name = "Click to remove/add Scatter Points",
          # hovertemplate = paste(
          #   "<b>%{text}</b><br><br>",
          #   "%{Religion in Daily Lives}: %{x:.0%}<br>",
          #   "%{Believe Region over science}: %{y:.0%}<br>",
          #   "<extra></extra>"
            )
          )
  })
